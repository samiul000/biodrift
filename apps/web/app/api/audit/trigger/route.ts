...
const COLLECTOR_CONFIG: Record<string, { id: string; url: string }> = {
  ...
  isrctn: {
    ...
    url: "https://www.isrctn.com/search?q=cancer",
  },
  cancer_research_uk: {
...
async function triggerCollector(
  ...
  const res = await fetch(
    `${API_BASE}/trigger?collector=${collectorId}&queue_next=1`,
    {
      method: "POST",
      headers: {
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      return {
        ...
        ok: true,
        records: data.length,
        message: `${name} (${data.length} records)`,
      };
    }
...
export async function POST(req: Request) {
  ...
  const results = await Promise.all(
    Object.entries(targets).map(([name, config]) =>
      healCollector(name, config, token),
    ),
  );
  ...
  if (healed.length === results.length) {
    ...
    message =
      "Collector returned empty results; batch may still be processing. Try again shortly.";
  }
